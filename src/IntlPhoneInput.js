...
export default class IntlPhoneInput extends React.Component {
  ...
  componentDidMount() {
    if (this.props.value) {
      const { phoneNumber } = this.setMask(
        this.props.value.replace(this.state.dialCode, "")
      );
  ...
  setMask = (value) => {
    ...
    return { phoneNumber, unmaskedPhoneNumber };
  };

  onChangeText = (value) => {
    let unmaskedPhoneNumberRaw = (value.match(/\d+/g) || []).join("");

    if (unmaskedPhoneNumberRaw.length === 0) {
      this.setState({ phoneNumber: "" });
      this.onChangePropText("", "");
      return;
    }

    const { phoneNumber, unmaskedPhoneNumber } = this.setMask(value);
